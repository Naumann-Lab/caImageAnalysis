...
def color_returner_continuous(val, theta, alphaStyle="default"):
    ...
    if alphaStyle == "default":
        for i in range(final_clrs.shape[1]):
            final_clrs[:, i, 3] = np.linspace(0, 1, 100)

    elif alphaStyle == "log":
        loggy = [np.log(i) for i in np.arange(0,100)]
        loggy = [i/np.max(loggy) for i in loggy]
        for i in range(final_clrs.shape[1]):
            final_clrs[:, i, 3] = loggy[i]
...
def make_clr_array(alphaStyle="default"):
    from colour import Color
    ...
    if alphaStyle == "default":
        ...
        loggy = [i/np.max(loggy) for i in loggy]
        for i in range(final_clrs.shape[1]):
            final_clrs[:, i, 3] = loggy

    elif alphaStyle == "exp":
        expy = [i**4 for i in np.arange(0,100)]
        expy = [i/np.max(expy) for i in expy]
        for i in range(final_clrs.shape[1]):
            final_clrs[:, i, 3] = expy
...
def continuous_clr_array(val, theta, clr_array):
    theta = int(theta % 360)  # get in 0-360 range
    theta = np.clip(theta, a_min=0, a_max=359)
    val = np.clip(int(val * 100), a_min=0, a_max=99)
