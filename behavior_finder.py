...
def saccade_finder(fish, time_window_s = 1, min_interval_s = .5, min_saccade_s = 0.1, startframe = 0, endframe = 10000,
    ...
    for e, eye in enumerate(eyelist):
        ...
        if lowpass:
            e_pass = utils.butter_lowpass_filter(fish.eye_df.loc[:, eye], cutoff = 2, fs = 1/eye_frame_s)#1-5
    ...
    if np.mean(fish.eye_df.th_e0) <= np.mean(fish.eye_df.th_e1):
        ...
        eye_saccade_df.loc[:, "e_conv_change"] = eye_saccade_df.e0_change - eye_saccade_df.e1_change
...
def plot_all_saccades(fish, stim_matrix):
    ...
    fig, ax = plt.subplots(stim_matrix.shape[0] * 10, stim_matrix.shape[1], dpi = 240, figsize = (8, 20))#assume maximum 10 trials per stim
    for row in range(stim_matrix.shape[0]):
        for col in range(stim_matrix.shape[1]):
            stim = stim_matrix[row][col]
            axes = [ax[row * 10 + i, col] for i in range(10)]
            if stim == 'skip':
                for axis in axes:
                    axis.set_axis_off()
            else:
...
def tail_finder(fish, time_window_s=.1, min_bout_s=0.05, min_interval_s=0.1, startframe=0, endframe=100000,
    ...
    for i in range(len(cont_tuples)):
        ...
        # ail_frequency_s[i] = np.divide(crossing / 2, tail_duration_s[i])
        stim_delay_s = np.subtract(cont_tuples_realtime_s[i][0], fish.stimulus_df.real_starttime_s)
        stim_delay_s = [t for t in stim_delay_s if t > 0]
        stim_nearest_row = np.argmin(stim_delay_s)  # all the stimulus happening before this tail
        stim_delay_s = stim_delay_s[stim_nearest_row]
        ...
        if type(stim_name) == list:  # overlap stimulus
            if stim_delay_s < min(stim_stationarytime):
...
def plot_boutangle_distribution(fish, stim_lists, s = 50, group = False, tail_bout_df = None,
    ...
    def fix_ax(ax, title):
        ...
        ax.set_xlim([np.deg2rad(90), np.deg2rad(-90)])
    ...
    fig, ax = plt.subplots(2, len(stim_lists), dpi=600, figsize=(20, 4), subplot_kw={'projection': 'polar'},
    ...
    tail_bout_df.loc[:, 'eye_vergence_norm'] = [(i - np.nanmin(tail_bout_df.eye_vergence)) / (
                np.nanmax(tail_bout_df.eye_vergence) - np.nanmin(tail_bout_df.eye_vergence)) for i in
                                                tail_bout_df.eye_vergence]
        ...
        if '[' not in stimlist[0] and 'dot' not in stimlist[0]:#grating
            ...
            for stim in stimlist:
                ...
                if len(stim_tail_bout_df) > 0:
                    ax[0, n].scatter(stim_tail_bout_df.tail_angle, stim_tail_bout_df.tail_duration_s,
                                  color=color, s=s, marker='.', linewidth=0, alpha = 1, zorder = 0)#alpha=stim_tail_bout_df.eye_vergence_norm, zorder = 0)
                    ax[0, n].scatter(np.mean(stim_tail_bout_df.tail_angle), np.mean(stim_tail_bout_df.tail_duration_s),
                                  edgecolor=color, facecolor='white', s=s, zorder = 1)
            ...
            for stim in stimlist:
                ...
                if len(first_half_bout_df) > 0:
                    alpha = np.subtract(1, np.divide(np.subtract(center_time, first_half_bout_df.tail_fromstart_s), 3))
                    alpha = np.clip(alpha, 0, 1)
                    ax[0, n].scatter(first_half_bout_df.tail_angle, first_half_bout_df.tail_duration_s,
                                  color=color, s=s, marker='.', linewidth=0,  alpha = alpha, zorder = 0)
                    #ax[0, n].scatter(np.mean(first_half_bout_df.tail_angle), np.mean(first_half_bout_df.tail_duration_s),
                    #              edgecolor=color, facecolor='white', s=s, zorder = 1)
                if len(second_half_bout_df) > 0:
                    alpha = np.subtract(1, np.divide(np.subtract(second_half_bout_df.tail_fromstart_s, center_time), 3))
                    alpha = np.clip(alpha, 0, 1)
                    ax[1, n].scatter(second_half_bout_df.tail_angle, second_half_bout_df.tail_duration_s,
                                  color=color, s=s, marker='.', linewidth=0, alpha=alpha, zorder = 0)
                    #ax[1, n].scatter(np.mean(second_half_bout_df.tail_angle), np.mean(second_half_bout_df.tail_duration_s),
                    #              edgecolor=color, facecolor='white', s=s, zorder = 1)
            fix_ax(ax[0, n], 'dot')
            fix_ax(ax[1, n], 'dot')
        else: #overlap
            for stim in stimlist:
                stim_tail_bout_df = tail_bout_df[tail_bout_df['tail_realstimuli'] == stim]
                color = [i for i in utils.stim_colors[stim.split("'")[3]]]
                stim_df = stimulus_df[stimulus_df.stim_name == stim].iloc[0]
                _, _, _, center_time = utils.get_dot_pos(stim_df)
                first_half_bout_df = stim_tail_bout_df[stim_tail_bout_df.tail_fromstart_s <= center_time]
                second_half_bout_df = stim_tail_bout_df[stim_tail_bout_df.tail_fromstart_s > center_time]
                if len(first_half_bout_df) > 0:
                    ...
                    ax[0, n].scatter(first_half_bout_df.tail_angle, first_half_bout_df.tail_duration_s,
                                     color=color, s=s, marker='.', linewidth=0,alpha=alpha, zorder=0)
                    # ax[0, n].scatter(np.mean(first_half_bout_df.tail_angle),
                    #                  np.mean(first_half_bout_df.tail_duration_s),
                    #                  edgecolor=color, facecolor='white', s=s, zorder=1)
                if len(second_half_bout_df) > 0:
                    alpha =  np.subtract(1, np.divide(np.subtract(second_half_bout_df.tail_fromstart_s, center_time), 3))
                    alpha = np.clip(alpha, 0, 1)
                    ax[1, n].scatter(second_half_bout_df.tail_angle, second_half_bout_df.tail_duration_s,
                                     color=color, s=s, marker='.', linewidth=0,alpha=alpha, zorder=0)
                    # ax[1, n].scatter(np.mean(second_half_bout_df.tail_angle),
                    #                  np.mean(second_half_bout_df.tail_duration_s),
                    #                  edgecolor=color, facecolor='white', s=s, zorder=1)
...
def plot_bouttime_distribution(fish, stim_lists, s = 50, group = False, tail_bout_df = None,
    ...
    fig, ax = plt.subplots(2, len(stim_lists), dpi=600, figsize=(20, 5), gridspec_kw={'wspace': 0.5})
    if not group:
        stimulus_df = fish.stimulus_df.copy()
        tail_bout_df = fish.bout_df.copy()
    stimulus_df.loc[:, 'stim_name'] = [str(i) for i in stimulus_df.stim_name]
    tail_bout_df.loc[:, 'tail_realstimuli'] = [str(i) for i in tail_bout_df.tail_realstimuli]
    tail_bout_df.loc[:, 'eye_vergence_norm'] = [(i - np.nanmin(tail_bout_df.eye_vergence) - 5) / (
                np.nanmax(tail_bout_df.eye_vergence) - np.nanmin(tail_bout_df.eye_vergence) - 5) for i in
                                                tail_bout_df.eye_vergence]
    tail_bout_df.loc[:, 'eye_vergence_norm'] = [i if i != np.nan else 0 for i in tail_bout_df.eye_vergence_norm]
    for n, stimlist in enumerate(stim_lists):
        ax_dot = ax[0, n].twinx()
        if '[' not in stimlist[0]:  # gratings or dot
            for stim in stimlist:
                ...
                color = ['slateblue' if 'dot_l' in s else 'mediumvioletred' if 'dot_r' in s else utils.stim_colors[s] for s in stim_tail_bout_df.tail_realstimuli]
                if len(stim_tail_bout_df) > 0:
                    ax[0, n].scatter(stim_tail_bout_df.tail_fromstart_s, stim_tail_bout_df.tail_angle,
                                  color=color, s=s, marker='.', linewidth=0, alpha=0.9, zorder=1)
                if 'dot' in stim:#put dot trajectory
                    ...
                    ax[0, n].axvline(center_time, linestyle='--', color='lightgrey')
                    ax[1, n].axvline(center_time, linestyle='--', color='lightgrey')
            ...
            for stim in stimlist:
                stim_tail_bout_df = tail_bout_df[tail_bout_df['tail_stimuli'] == stim]
                color =['slateblue' if 'dot_l' in s else 'mediumvioletred' if 'dot_r' in s else utils.stim_colors[s] for s in stim_tail_bout_df.tail_realstimuli]
                #[utils.stim_colors[s] if '[' not in s else utils.stim_colors[s.split("'")[3]] for s in stim_tail_bout_df.tail_realstimuli]
                #['slateblue' if 'dot_l' in s else 'mediumvioletred' if 'dot_r' in s else utils.stim_colors[s] for s in stim_tail_bout_df.tail_realstimuli]
        ...
        for stims in [dotl_stims, dotr_stims, other_stims]:
            if len(stims) > 0:
                if 'dot' in stims[0]:  # put dot trajectory
                    if 'dot_l' in stims[0]:
                        stim_color = 'slateblue'
                    elif 'dot_r' in stims[0]:
                        stim_color = 'mediumvioletred'
                else:
                    stim_color = 'black'
                ...
                if len(l_bout_df) > 0:
                    freq, bin_loc = np.histogram(l_bout_df.tail_fromstart_s, bins=30, range=[0, 35])
                    bin_center = (bin_loc[:-1] + bin_loc[1:]) / 2
                    ax[1, n].plot(bin_center, np.multiply(freq, 2), color = stim_color, linestyle = ':', zorder = 1)
                r_bout_df = row_bout_df[row_bout_df.tail_angle > 0]
                if len(r_bout_df) > 0:
                    freq, bin_loc = np.histogram(r_bout_df.tail_fromstart_s, bins=30, range=[0, 35])
                    bin_center = (bin_loc[:-1] + bin_loc[1:]) / 2
                    ax[1, n].plot(bin_center, np.multiply(freq, 2), color= stim_color, zorder=1)
            ax[1, n].set_ylim([0, 40])
            ax[1, n].set_xlim([0, 15])
