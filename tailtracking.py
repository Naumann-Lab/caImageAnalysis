...
def dateToMillisec(datetime):
    return datetime.microsecond/1000 + datetime.second*1000 + datetime.minute *60 *1000 + datetime.hour *60 *60 *1000
...
def tail_reader(tail_path): #reads in the tail data into a df
    ...
    tail_df.loc[:, 't'] = tail_df["/'TailLoc'/'Time'"].values - tail_df["/'TailLoc'/'Time'"].values[0]

    t_arr = []
    for t in range(len(tail_df.t.values)):
        t_arr.append(np.timedelta64(tail_df.t.values[t], 'ms').astype(int))
    tail_df['t'] = t_arr
    tail_df["/'TailLoc'/'Time'"] = tail_df["/'TailLoc'/'Time'"].dt.tz_localize('US/Eastern')

    #add extra column at the end with the converted time
    tail_ts = []
    for i in range(len(tail_df)):
        try:
            val = dt.strptime(str(tail_df["/'TailLoc'/'Time'"].iloc[i]).split(' ')[1].split('-')[0],
                              '%H:%M:%S.%f').time()
        except:
            val = dt.strptime(str(tail_df["/'TailLoc'/'Time'"].iloc[i]).split(' ')[1].split('-')[0],
                          '%H:%M:%S').time()
    ...
    for i in range(len(tailTimes)):
        tailTimes[i] = tailTimes[i].replace(hour = tailTimes[i].hour - 5, minute = tailTimes[i].minute,
                                            second = tailTimes[i].second, microsecond = tailTimes[i].microsecond)
