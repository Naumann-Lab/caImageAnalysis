...
def pathSorter(parentPath=None, key=None):
        ...
        for entry in entries:
            if entry.is_file():
                ...
                if entry.name.endswith("Params.json"):
                    _paths["etc"]["moveCorrectionParams"] = Path(parentPath).joinpath(
                        entry.name
                    )
                ...
                if entry.name.endswith("Timestamps.tdms"):
                    _paths["image"]["timestamps"] = Path(parentPath).joinpath(
                        entry.name
                    )
                if "timestamps.txt" in entry.name:
                    _paths["image"]["new_timestamps"] = Path(parentPath).joinpath(
                        entry.name
                    )

                if "frametimes.txt" in entry.name:
                    _paths["image"]["anne_timestamps"] = Path(parentPath).joinpath(
                        entry.name
                    )

                if "frametimes.h5" in entry.name:
                    _paths["image"]["new_timestamps_processed"] = Path(
                        parentPath
                    ).joinpath(entry.name)

                if (
                    "stims" in entry.name or entry.name.startswith("fish")
                ...
                if entry.name.endswith(".h5") and entry.name.startswith("fish"):
                    _paths["stimuli"]["processed"] = Path(parentPath).joinpath(
                        entry.name
                    )
                if entry.name.endswith("aligned.h5"):
                    _paths["stimuli"]["frame_aligned"] = Path(parentPath).joinpath(
                        entry.name
                    )
                ...
                if "eigen" in entry.name:
                    _paths["output"]["eigenvalues"] = Path(parentPath).joinpath(
                        entry.name
                    )
            else:
                if entry.name == "timing":
                    _paths["image"]["new_timestamps"] = Path(parentPath).joinpath(
                        "timing/framesendtimes.txt"
                    )
                if entry.name == "suite2p":
                    _paths["output"]["suite2p"] = Path(parentPath).joinpath(
                        "suite2p/plane0"
                    )
    ...
    if moveCorrectedPath.exists():
        with os.scandir(moveCorrectedPath) as entries:
            for entry in entries:
                if entry.name.endswith(".tif"):
                    _paths["image"]["move_corrected"] = Path(
                        moveCorrectedPath
                    ).joinpath(entry.name)
...
def load_suite2p(_paths):
    try:
        ...
        iscell = np.load(_paths.joinpath("iscell.npy"), allow_pickle=True)[:, 0].astype(
            bool
        )
...
def stimStartStop(stims):
    ...
    stim_dict = {"stimulus": stimuli, "start": starts, "stop": stops}
...
def threshold_by_variance(eigen, variance=0.5):
    ...
    loading_mtrx = pd.DataFrame(
        loadings, columns=["FA{}".format(i) for i in range(0, 10)], index=x.columns
    )
...
def pandas2hdf(_path):
    ...
    if "tex_freq" in stimulus_details[0]:
        ...
        stimulus_dicts = [
            eval(i[i.find("{") :]) for i in _stimulus_dicts if "stationary_end" not in i
        ]
        freq_fixer = []
        ...
        stimulus_dicts = [
            eval(i[i.find("{") : int(i.find("texture_0")) - 1] + "}")
            for i in stimulus_details
            if "stationary_end" not in i
        ]
    else:
        stimulus_dicts = [
            eval(i[i.find("{") :])
    ...
    for stim in range(len(stimulus_dicts)):
        for item in stimulus_dicts[stim].copy():
            try:
                if (
                    len(stimulus_dicts[stim][item]) > 1
                    and type(stimulus_dicts[stim][item]) is not str
                ):
...
def pandas_stim_aligner(_path, anne_timestamps=False, mapstims=False):
    ...
    stims["raw_t"] = stims["raw_t"].apply(
        lambda a: dt.strptime(a, "%H:%M:%S.%f").time()
    )
    ...
    if anne_timestamps:
        ...
        try:
            ...
            try:
                image_times = (
                    pd.read_hdf(paths["image"]["new_timestamps_processed"])
                    .rename({0: "times"}, axis=1)
                    .reset_index()
                )
    ...
    if new_timestamps:
        ...
        for i in range(len(image_times) - 2):
            diffs.append(
                dt.combine(date.today(), image_times.iloc[i + 1].values[0])
                - dt.combine(date.today(), image_times.iloc[i].values[0])
            )
    ...
    try:
        end_ind_offset = (stims.iloc[-1].duration - stims.iloc[-1].stationary_time) // (
            imageHz / 1000
        )
    except AttributeError:
        _statval = max(
            (stims.iloc[-1].duration - stims.iloc[-1].stationary_time_0),
            (stims.iloc[-1].duration - stims.iloc[-1].stationary_time_1),
        )
        end_ind_offset = _statval // (imageHz / 1000)

    try:
        end_ind = (
            image_times[image_times.times >= stims.iloc[-1].raw_t].index[0]
            + end_ind_offset
        )
    except IndexError:
    ...
    for i in range(stim_num):
        if i + 1 >= stim_num:
            break
        indices = image_times[
            (image_times.times >= stims.loc[i].raw_t)
            & (image_times.times <= stims.loc[i + 1].raw_t)
        ].index
        imgs.append(indices)
    imgs.append(
        image_times[(image_times.times >= stims.loc[stim_num - 1].raw_t)]
        .loc[:end_ind]
        .index
    )
    try:
        ...
        for i in stims.raw_t:
            new_raw_t.append(
                dt.strptime(i.strftime("%I:%M:%S.%f:"), "%I:%M:%S.%f:").time()
            )
        ...
        for i in range(stim_num):
            if i + 1 >= stim_num:
                break
            indices = image_times[
                (image_times.times >= stims.iloc[i].raw_t)
                & (image_times.times <= stims.iloc[i + 1].raw_t)
            ].index
            imgs.append(indices)
        stims = stims.iloc[1:]
        stims.loc[:, "img_stacks"] = imgs
...
def image_timings(frameTimePath):
    ...
    try:
        ...
        for i in range(len(time_data)):
            time_array.append(
                pd.to_datetime(time_data[i], utc=True)
...
def log_aligner(logsteps, frametimes):
    ...
    trimmed_logsteps = logsteps[
        (logsteps.times >= frametimes.iloc[0].values[0])
        & (logsteps.times <= frametimes.iloc[-1].values[0])
    ]
...
def stackHzReturner(timeframe_df):
    ...
    for i in range(len(t_vals) - 1):
        times.append(
            dt.combine(dt_offset, t_vals[i + 1]) - dt.combine(dt_offset, t_vals[i])
        )
...
def returnFoverF(stim_df, good_cells):
    ...
    try:
        for arr in stim_df[
            (stim_df.velocity_0 == 0) & ((stim_df.velocity_1 == 0))
        ].img_stacks.values:
            for j in arr[5:-5]:
                inds.append(j)
    except AttributeError:
        for arr in stim_df[
            (stim_df.velocity == 0) & ((stim_df.velocity == 0))
        ].img_stacks.values:
            for j in arr[5:-5]:
                inds.append(j)
        ...
        for stimulus in mstim_df.stimulus.unique():
            ...
            nrn_vals.append(
                np.mean(
                    cells[
                        cell,
                        arrangedArrays(mstim_df[mstim_df.stimulus.isin([stimulus])]),
                    ]
                )
            )
...
def stim_cell_returner(base_path, mapraw=False, fix_stims=False, s2p=False):
    ...
    if binoc:
        try:
            ...
            inds_2 = stimuli[
                (stimuli.velocity_0 == 0) & (stimuli.velocity_1 == 0)
            ].index
    ...
    if s2p is not False:
        ...
        ops, iscell, stats, f_cells = load_suite2p(
            pathSorter(base_path)["output"]["suite2p"]
        )
...
def returnStimCellAligned(stims_all, cells_all, leading_frames=50):
    ...
    for b in range(len(cells_all)):
        ...
        fin_cells.append(
            cells_all[b][:, rel_inds[b][0] - leading_frames + offset_l : rel_inds[b][1]]
        )
...
def tail_reader(tail_data_path):
    ...
    for i in range(len(df)):
        tail_ts.append(
            dt.strptime(
                str(df["/'TailLoc'/'Time'"].iloc[i]).split(" ")[1].split("-")[0],
                "%H:%M:%S.%f",
            ).time()
        )
...
def tail_alignment(basePath):
    ...
    trimmedTailData = fullTailData[
        (fullTailData.conv_t >= imageTimes.values[0][0])
        & (fullTailData.conv_t <= imageTimes.values[-1][0])
    ]
    for frameN in range(len(imageTimes.values)):
        try:
            indices = trimmedTailData[
                (trimmedTailData.conv_t >= imageTimes.values[frameN][0])
                & (trimmedTailData.conv_t <= imageTimes.values[frameN + 1][0])
            ].index
        except IndexError:
            pass
...
def return_stim_responses(
    stimulus_df: pd.DataFrame,
    cell_array: np.array,
    background_response: np.array,
    offset: int = 12,
) -> np.array:
    """
...
def return_stim_response_dict(
    data_path: str,
    stim_start_offset: int = -3,
    stim_stop_offset: int = 7,
    ...
    assert (
        response_type in accepted_response_types
    ), f"response type must be in {accepted_response_types}"

    stuff = pathSorter(data_path)
    stim = pd.read_hdf(stuff["stimuli"]["frame_aligned"])
    ops, iscell, stats, f_cells = load_suite2p(
        stuff["stimuli"]["frame_aligned"].parents[0].joinpath("suite2p/plane0")
    )
    norm_cells = normcells(f_cells)
    convolved_arrs = np.array(
        [
            np.convolve(norm_cells[i], np.ones(100) / 100, "same")
    ...
    stimulus_stops = [
        i[stim_start_offset] + stim_stop_offset for i in stim.img_stacks.values
    ]
    ...
    for stimChoice in stim.stim_name.unique():
        ...
        stimframes = np.unique(
            np.clip(np.concatenate(stimFrames), a_min=0, a_max=norm_cells.shape[1] - 1)
        )
        ...
        if response_type == "mean":
            ...
            responseDict[stimChoice] = np.nanmean(
                norm_cells[:, stimframes], axis=1
            ) - np.nanmean(convolved_arrs[:, stimframes], axis=1)
...
def better_volume_splits(base_path, len_thresh=150):
    ...
    frametimes = other_utils.raw_text_frametimes_to_df(
        loc_paths["image"]["new_timestamps"]
    )
    log_steps = other_utils.raw_text_logfile_to_df(loc_paths["image"]["log"])
    frametimes = other_utils.alignmentFramesSteps(
...
def return_thresholded(
    base_path, threshold=0.3, color_set="full", response_type="mean"
):
    ...
    assert (
        color_set in accepted_color_sets
    ), f"color type must be one of {accepted_color_sets}"
    ...
    for z in range(len(plane_list)):
        ...
        peak_df = pd.DataFrame(
            {"neuron": neuronVals, "stimulus": keyVals, "response": maxVals}
        )
...
def return_locationed(base_path, threshold=0.3):
    ...
    for i, plane in enumerate(plane_list):
        ...
        colors = [
            visualize.eva_weightings[neuron] for neuron in neuron_set.stimulus.values
        ]

        pos_df = pd.DataFrame(
            {
                "stimulus": neuron_set.stimulus.values,
                "x": x_pos,
                "y": y_pos,
                "color": colors,
            }
...
def new_fdff(stimuli, f_cells, buffer=2):
    ...
    baseline_period = (
        np.min(
            [
                stimuli.start.values[i] - stimuli.stop.values[i - 1]
                for i in range(len(stimuli))
                if i > 0
            ]
        )
        - buffer
    )  # use two frame min buffer
    fdff_cells = []
    for cell_n in range(f_cells.shape[0]):
        bg_val_n = np.nanmean(
            [
                f_cells[cell_n][
                    stimuli.iloc[stim_n].start
                    - baseline_period : stimuli.iloc[stim_n].start
                    - 1
                ]
                for stim_n in range(len(stimuli))
            ]
        )
        fdff_cell = (f_cells[cell_n] - [bg_val_n] * len(f_cells[cell_n])) / np.nanmax(
            (f_cells[cell_n] - [bg_val_n] * len(f_cells[cell_n]))
        )
...
def new_fdff_avg_all(stimuli, f_cells):
    ...
    for cell_n in tqdm(range(f_cells.shape[0])):
        for stim_n in range(len(stimuli) - 1):
            ...
            pltarr = f_cells[cell_n][
                stimuli.iloc[stim_n]
                .img_stacks.values[0] : stimuli.iloc[stim_n]
                .img_stacks.values[-1]
                + baseline_period
            ]
            ...
            real_arr_inds.append(
                np.arange(
                    stimuli.iloc[stim_n].img_stacks.values[0],
                    stimuli.iloc[stim_n].img_stacks.values[-1] + baseline_period,
                )
            )
        ...
        f_Df = pd.DataFrame(
            {
                "corr_index": np.concatenate(real_arr_inds),
                "fdff": np.concatenate(all_arrs),
            }
        )
...
def new_fdff_all_stims(stimuli, f_cells):
    ...
    for cell_n in tqdm(range(f_cells.shape[0])):
        ...
        f_Dfs = pd.DataFrame(
            {"stimulus": all_stims, "fdff": all_arrs, "corr_index": real_arr_inds}
        )
...
def make_predictions(stim_fdff, stimuli, split_frac=0.66, trimStim=False):
    ...
    if trimStim:
        stimuli = stimuli[
            stimuli.stim_name.isin(["left", "right", "forward", "backward"])
        ]
    ...
    for stim in stimuli.index.values[:-1]:
        ...
        maxVals = np.clip(
            np.nan_to_num([i.max() for i in stub.fdff.values]), a_min=-1, a_max=1
        )
    ...
    LR = LogisticRegression(
        random_state=0, solver="lbfgs", multi_class="multinomial", max_iter=1000
    ).fit(x_tr, y_tr)
    ...
    RF = RandomForestClassifier(n_estimators=1000, max_depth=10, random_state=0).fit(
        x_tr, y_tr
    )
    ...
    return pd.DataFrame(
        {
            ...
            "SupportVectorMachine": [svm_score],
