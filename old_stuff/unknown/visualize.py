...
def color_generator(good_loadings, c1=None, c2=None):
    ...
    if c1 and c2 is not None:
        clrs = list(
            Color(c1).range_to(Color(c2), good_loadings.highest_loading.nunique())
        )
        corr_colors = [i.rgb for i in clrs]
    else:
        corr_colors = sns.color_palette(
            n_colors=good_loadings.highest_loading.nunique()
        )
...
def plot_factor(_path, factors=[0, 1, 2], variance=0.5):
    ...
    c_lines = [
        Line2D([0], [0], color=factor_colors[i], lw=4)
        for i in range(len(factor_colors))
    ]

    ax.legend(
        c_lines,
        ["factor " + str(i) for i in range(len(factor_colors))],
        loc="upper right",
        fontsize="x-large",
    )
    return
...
def full_plot(_path, save=None, age=None, variance=0.5):
    ...
    for val in range(goodLoadings.highest_loading.nunique()):
        a.append(
            len(
                goodLoadings[
                    goodLoadings.highest_loading
                    == goodLoadings.highest_loading.unique()[val]
                ]
            )
        )
    ...
    for j in [a1, a2, a3]:
        for i in range(len(stim_df)):
            j.axvspan(
                stim_df.iloc[i].start,
                stim_df.iloc[i].stop,
                color=stim_df.iloc[i].color,
                alpha=0.3,
...
def plot_avg_stimulus(_path, stim=["Rx"], sig=0.7, offset=5, threshold=2.0):
    ...
    for nrn in range(f_cells[iscell].shape[0]):
        ...
        if (
            np.mean(neuron[s + offset : e + offset])
            / np.mean(neuron[s - 2 * offset : s])
            >= threshold
        ):
    ...
    for i in selection:
        neurons.append(
            (f_cells[iscell][i] - np.mean(f_cells[iscell][i][:15]))
            / np.mean(f_cells[iscell][i][:15])
        )
    ...
    for i in range(stim_df.stimulus.nunique()):
        ...
        for ind in range(len(plot_df)):
            ax[ind1, ind2].plot(
                np.mean(
                    neurons[
                        :,
                        plot_df.start.values[ind]
                        - offset : plot_df.stop.values[ind]
                        + offset,
                    ],
                    axis=0,
                ),
        ...
        ax[ind1, ind2].axvspan(
            offset,
            offset
            + abs(
                plot_df.start.values[ind] - offset - plot_df.stop.values[ind] + offset
            ),
            color="red",
            alpha=0.2,
        )
...
def plot_avg_stimulus_neurons(_path, stim=["Rx"], sig=0.7, offset=5, threshold=2.0):
    ...
    fig, ax = plt.subplots(
        neurons.shape[0], figsize=(neurons.shape[0], neurons.shape[0])
    )
    ...
    for i in range(neurons.shape[0]):
        ...
        for x in range(len(plot_df)):
            ax[i].axvspan(
                plot_df.start.values[x] + offset,
                plot_df.stop.values[x] + offset,
                color="red",
                alpha=0.2,
            )
...
def pixelwise(base_path, stimOffset=5, local_bright=1.5):
    ...
    try:
        if "stim_name" in frameAlignedStims.columns:
            frameAlignedStims.rename(
                columns={"stim_name": "stimulus_name"}, inplace=True
            )
    ...
    for stim in stim_df[stim_df.stimulus.isin(eva_weightings.keys())].stimulus.unique():
        _frames = utils.arrangedArrays(
            stim_df[stim_df.stimulus == stim], offset=stimOffset
        )
        ...
        all_img.append(
            np.dstack((r**local_bright, g**local_bright, b**local_bright))
        )
...
def pixelwise2(base_path, stimOffset=5):
    ...
    frames, x, y = img.shape
    stim_df.stimulus = stim_df.stimulus.map(
        {"F": "Forward", "RR": "Right", "LL": "Left", "B": "Backward"}
    )
...
def pixelwisevolumes(planePath):
    ...
    return (
        fin_img,
        _all_img,
        stim_df[stim_df.stimulus.isin(eva_weightings.keys())].stimulus.unique(),
    )
...
def spatial_neurons(base_path, x=[0, 9999], y=[0, 9999]):
    ...
    return locs[
        (locs.x >= x[0]) & (locs.x <= x[1]) & ((locs.y >= y[0])) & ((locs.y <= y[1]))
    ]
...
def stimResponses(cells, stims, threshold=0.5, offset=10):
    ...
    celldf = pd.DataFrame(
        {"neuron": neurons, "stimulus": stimuli, "val": finVals, "error": finVals_std}
    )
...
def neuronColor(
    ...
    if volume is not False:
        ...
        ops, iscell, stats, f_cells = utils.load_suite2p(
            Path(basePath).joinpath("suite2p/plane0")
        )
        stim_df, cells = utils.stim_cell_returner(
            basePath, s2p=[ops, iscell, stats, f_cells]
        )
...
def topneuroncolor(
    basePath, threshold=0.5, offset=5, z_scored=False, return_stuff=False
):
    ops, iscell, stats, f_cells = utils.load_suite2p(
        utils.pathSorter(basePath)["output"]["suite2p"]
    )
    ...
    tunedCells, celldf, iscell, stats, f_cells, mstim_df, stim_df = neuronColor(
        basePath,
        threshold=threshold,
        offset=offset,
        plot=False,
    ...
    for nrn in tunedCells.neuron.unique():
        pref = (
            tunedCells[tunedCells.neuron == nrn]
    ...
    if annotating:
        for cell in plottedCells:
            plt.annotate(
                cell,
                (
                    cell_img.shape[0] - np.mean(stats[cell]["ypix"]),
                    np.mean(stats[cell]["xpix"]),
                ),
                color="white",
                weight="bold",
                size=12,
...
def active_passive_plot(basePath, stim="forward", sig=3, abr_cutoff=75):
    ...
    smoothedVals = scipy.ndimage.gaussian_filter(
        tail_data["/'TailLoc'/'TailCurvature'"].values, sigma=sig
    )

    rollingVals = (
        pd.Series(full_tail["/'TailLoc'/'TailCurvature'"].values)
        .rolling(1000)
        .median()[tail_data.index]
    )
    ...
    try:
        staticIndices = aligned_stims[
            (aligned_stims.velocity_0 == 0) & (aligned_stims.velocity_1 == 0)
        ].index
    except:
        staticIndices = aligned_stims[aligned_stims.velocity == 0].index

    statInds = []
    for j in aligned_stims[
        (aligned_stims.velocity_0 == 0) & (aligned_stims.velocity_1 == 0)
    ].img_stacks:
        for i in j:
            statInds.append(i)
    ...
    for n in range(len(b_stims)):
        n_inds = tail_data[
            tail_data.frame.isin(
                np.arange(b_stims.iloc[n].start - 1, b_stims.iloc[n].stop)
            )
        ].index
...
def monocplot(
    ...
    if not suite2p:
        ops, iscell, stats, f_cells = utils.load_suite2p(
            Path(planepath).joinpath("suite2p/plane0")
        )
    else:
    ...
    cell_percentiles = np.nanquantile(
        f_cells, background_percentile, axis=1, keepdims=True
    )
    ...
    nrns = np.unique(
        np.array(sorted(np.where(np.array(stimResponses) >= tuned_thresh)[1]))
    )
...
def radial_nrn_plot(
    ...
    maxdf = pd.DataFrame(
        {"neuron": nrns, "direction": dirs, "velocity": vels, "frequency": freqs}
    )
    ...
    if plot:
        ...
        jitteryness = [1, 1]  # X / Y
        # radial jitter
        maxdf.direction = maxdf.direction + [
            2.5 * jitteryness[0] * radians(x)
            for x in np.random.normal(0, 1, len(maxdf))
        ]
        # value jitter
        maxdf.velocity = maxdf.velocity + np.random.normal(0, 1, len(maxdf)) / (
            400 * jitteryness[1]
        )
...
def resp_plot(base_path, palette, offset=12, nrn=0):
    ...
    plt.scatter(
        np.arange(0, len(stim_responses[:, nrn])),
        stim_responses[:, nrn][d.index.values],
    )
    for n, ang in enumerate(d.angle.values):
        plt.axvline(x=n, color=palette[ang], alpha=0.5)

    legend_lines = [
        Line2D([0], [0], color=palette[clr], marker="o") for clr in palette.keys()
    ]
...
def response_hists(planePath, returns=False):
    ...
    try:
        stim_starts = [
            stimuli.img_stacks.values[i][-offset_val]
            for i in range(stimuli.img_stacks.values.shape[0])
        ]
    except IndexError:
        stim_starts = [
            stimuli.img_stacks.values[i][-3]
            for i in range(stimuli.img_stacks.values.shape[0])
        ]

    stim_stops = [
        stimuli.img_stacks.values[i][-1] + offset_val
        for i in range(stimuli.img_stacks.values.shape[0])
    ]
    stim_arrs = [
        np.clip(
            np.arange(stim_starts[i], stim_stops[i]), a_min=0, a_max=image.shape[0] - 1
        )
    ...
    bg_arrs = [
        np.clip(
            np.arange(
                stim_starts[i] - (2 * offset_val), stim_starts[i] - (offset_val // 2)
            ),
            a_min=0,
    ...
    cell_responses = {}
    background = np.mean(
        norm_f_cells[:, np.concatenate(stimuli.bg_frames.values)], axis=1
    )
    for stim in stimuli.stim_name.unique():
        stim_indices = np.concatenate(stimuli[stimuli.stim_name == stim].frames.values)
        cell_responses[stim] = np.clip(
            np.mean(norm_f_cells[:, stim_indices], axis=1) - background,
            a_min=0,
            a_max=1,
        )
...
def graph_legend(stimuli, bbox=[1.2, 0.5], lstyle=None):
    legend_lines = [
        Line2D([0], [0], color=clr, marker="o", linestyle=lstyle)
        for k, clr in eva_weightings.items()
        if k in stimuli.stim_name.unique()
    ]
    legend_labels = [
        str(k) for k, clr in eva_weightings.items() if k in stimuli.stim_name.unique()
    ]
    plt.legend(
        legend_lines, legend_labels, loc="right", bbox_to_anchor=bbox, title="stimulus"
    )
...
def graph_label(spanner, stimuli):
    ...
    for stim_n in spanner:
        plt.axvspan(
            ...
            stimuli.iloc[stim_n].img_stacks.values[-1],
            color=eva_weightings[stimuli.iloc[stim_n].stim_name],
            alpha=0.1,
        )
        plt.axvspan(
            stimuli.iloc[stim_n].img_stacks.values[-6],
            stimuli.iloc[stim_n].img_stacks.values[-1],
            color=eva_weightings[stimuli.iloc[stim_n].stim_name],
            alpha=0.7,
        )
    ...
    plt.axvspan(
        stimuli.iloc[stim_n + 1].img_stacks.values[0],
        stimuli.iloc[stim_n + 1].img_stacks.values[-1],
        color=eva_weightings[stimuli.iloc[stim_n + 1].stim_name],
        alpha=0.1,
    )
    plt.xlim(
        stimuli.iloc[spanner[0]].img_stacks.values[0],
        stimuli.iloc[spanner[-1]].img_stacks.values[-1] + (1.5 * baseline_period),
    )
...
def neuron_blast(
    stimuli, stim_fdff, chosen_neuron, highlights=[0, 15, 20], redlining=True
):
    subplot_locations = {
        "lateral_left": [0, 3],
        "lateral_right": [1, 3],
    ...
    if hasattr(chosen_neuron, "__iter__"):
        myDictionary = create_myDictionary(
            stimuli, stim_fdff[stim_fdff.neuron.isin(chosen_neuron)]
        )
    else:
        myDictionary = create_myDictionary(
            stimuli, stim_fdff[stim_fdff.neuron == chosen_neuron]
        )
    ...
    for k, v in subplot_locations.items():
        ...
        ax[v[0], v[1]].axvspan(
            highlights[0], highlights[1], alpha=0.1, color=eva_weightings[k]
        )
        ax[v[0], v[1]].axvspan(
            highlights[1], highlights[2], alpha=0.6, color=eva_weightings[k]
        )
        ax[v[0], v[1]].set_ylim(-0.5, 1.0)
        if redlining:
...
class ImagingDataVolumetric:
    ...
    def load_s2p(self):
        for plane in self.plane_list:
            ops, iscell, stats, f_cells = utils.load_suite2p(
                utils.pathSorter(plane)["stimuli"]["frame_aligned"]
                .parents[0]
                .joinpath("suite2p/plane0")
            )
    ...
    def create_threshdf(self, method="background_subtracted", used_threshold=0.1):
        ...
        for plane in self.plane_list:
            ...
            temp = (
                df[df >= used_threshold].isna().sum(axis=1)
                < self.datum_dict[plane]["stimuli"].stim_name.nunique()
            )
    ...
    def blast(self, plane, neurons, pretty=True, highlights=(0, 15, 20)):
        ...
        if hasattr(neurons, "__iter__"):
            myDictionary = create_myDictionary(
                local_stim, local_fdff[local_fdff.neuron.isin(neurons)]
            )
        else:
            myDictionary = create_myDictionary(
                local_stim, local_fdff[local_fdff.neuron == neurons]
            )
        ...
        for n, ax in enumerate(axes):
            ...
            ax.axvspan(
                highlights[1], highlights[2], alpha=0.6, color=eva_weightings[ax_stim]
            )
            ax.set_ylim(-0.5, 1.0)
            ax.set_xlim(13, 25)
    ...
    def plot_nclasses(self, plane, top_n, bbox=(2, 0.5), pretty=False, invert=True):
        ...
        legend_lines = [
            Line2D([0], [0], color=clr, marker=" ", linestyle=None)
            for clr in used_colors
        ]
        legend_labels = [k for k in value_df.index.values[:top_n]]
        plt.legend(
            legend_lines, legend_labels, loc="right", bbox_to_anchor=bbox, title="set"
        )
